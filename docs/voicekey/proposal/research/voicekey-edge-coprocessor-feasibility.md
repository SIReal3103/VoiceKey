...
iOS can support phone-only microphone and speaker use through `AVAudioSession`. A custom compute accessory is a separate risk. Apple now publishes a preliminary `AccessoryAccess` framework for connected USB accessories, while its established External Accessory documentation is associated with MFi accessories. The preliminary framework needs validation and is not a product commitment for the first prototype. [Apple AVAudioSession](https://developer.apple.com/documentation/avfaudio/avaudiosession/category-swift.struct/playandrecord) [Apple AccessoryAccess](https://developer.apple.com/documentation/AccessoryAccess) [Apple External Accessory](https://developer.apple.com/documentation/externalaccessory/)
